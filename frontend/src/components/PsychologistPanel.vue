    ...
    <div class="two-columns">
      ...
      <div class="card">
        ...
        <div v-for="t in tasks" :key="t.id" class="task-item">
          <div class="task-info">
            <span class="task-level level-2">二级</span>
            <span class="task-status">{{ t.status }}</span>
            <span class="task-assigned">👤 {{ t.assignedTo }}</span>
          </div>
          <div class="task-actions" v-if="t.status === 'pending'">
            <button class="btn-action good" @click="handlePsychologistDecision('好转')">✅ 好转结束</button>
            <button class="btn-action warn" @click="handlePsychologistDecision('需联系家长')">📞 未见好转，联系家长</button>
          </div>
        </div>
      </div>
...
const analysisForm = ref({
  academicPressure: '', socialRelationship: '', emotionalState: '',
  selfCognition: '', expertReport: '', extra: ''
})

const loadTasks = async () => {
  const res = await axios.get(`http://localhost:8080/api/task/${currentStudent.value}`)
  // 只显示二级任务
  tasks.value = res.data.filter(t => t.level === 2)
}
...
const loadChart = async () => {
  ...
  chart.setOption({
    ...
    series: [{ type: 'line', data: res.data.map(i => i.score), smooth: true, lineStyle: { color: '#4F46E5', width: 3 }, areaStyle: { opacity: 0.1, color: '#818CF8' }, symbol: 'circle', symbolSize: 6, itemStyle: { color: '#4F46E5' } }]
  })
}
...
const handlePsychologistDecision = async (decision) => {
  await axios.post('http://localhost:8080/api/task/psychologist/complete', {
    studentId: currentStudent.value, decision
  })
  if (decision === '需联系家长') {
    await axios.post('http://localhost:8080/api/note', {
      studentId: currentStudent.value, role: '系统',
      content: `【系统通知】心理老师评估：学生 ${currentStudent.value} 未见好转，建议班主任联系家长并进行心理治疗。`
    })
    alert('已通过系统通知提醒班主任联系家长')
  } else {
    alert('心理老师认为已好转，流程结束')
  }
  await refreshData()
}
const submitNote = async () => {
  ...
  await axios.post('http://localhost:8080/api/note', {
    studentId: currentStudent.value, role: '心理老师', content: noteContent.value
  })
  noteContent.value = ''
  await loadNotes()
}
const submitAnalysis = async () => {
  ...
  await axios.post('http://localhost:8080/api/analysis', {
    studentId: currentStudent.value, role: '心理老师',
    ...form
  })
  alert('分析记录已保存')
